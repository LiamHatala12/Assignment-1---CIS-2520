...
int
printHapax(char *filename, LLNode *wordListHeads[], int maxLen, int hapaxLength)
{
    int hapaxCount = 0;
    
    printf("Hapax legomena of length %d from file '%s':\n", hapaxLength, filename);

    // Iterate through the word lists
	for (int i = hapaxLength; i <= maxLen; i++) {
        LLNode *node = wordListHeads[i];
        while (node != NULL) {
            // Check if the word occurs only once and has the specified length
            if (node->count == 1 && strlen(node->word) == hapaxLength) {
                printf("    '%s'\n", node->word);
                hapaxCount++;
            }
            node = node->next;
    ...
    if (hapaxCount == 0) {
        printf("    No hapax legomena of length %d found in the file.\n", hapaxLength);
    }
